...
export default function EvacuationPage() {
  ...
  const handleMapReady = useCallback((map: google.maps.Map) => {
    ...
    // Shelter markers
    RESOURCES.filter(r => r.type === 'shelter').forEach(shelter => {
      const iw = new google.maps.InfoWindow({
        content: `<div style="background:#0d1117;color:#e2e8f0;padding:10px;border-radius:8px;font-family:monospace;min-width:180px">
          ...
          <div style="font-size:10px;color:${shelter.status === 'available' ? '#34d399' : '#fbbf24'};font-weight:700;margin-top:4px">${shelter.status.toUpperCase()}</div>
        </div>`,
      });
          ...
          <div className="flex items-start gap-2">
            ...
            <div>
              ...
              <p className="text-[10px] text-slate-400 leading-relaxed">
                Share your location to get real-time routes to all {RESOURCES.filter(r => r.type === 'shelter').length} Tampa Bay shelters, ranked by safety score. Routes avoid storm surge zones and account for live traffic. Auto-refreshes every 2 minutes.
              </p>
            </div>
          </div>
        </div>
