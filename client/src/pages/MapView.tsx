...
import { RESOURCES, type Resource } from '@/lib/stormData';
...
function zoneInfoContent(zone: LiveZone, cfg: { fill: string; stroke: string; label: string }) {
  return `
    <div style="min-width:220px;padding:14px;border-radius:16px;border:1px solid rgba(226,232,240,0.14);background:linear-gradient(180deg,rgba(255,255,255,0.1),rgba(255,255,255,0.03)),rgba(6,12,24,0.9);backdrop-filter:blur(16px);color:#e2e8f0;font-family:Inter,Arial,sans-serif;box-shadow:0 18px 45px rgba(2,6,23,0.35);">
      <div style="display:flex;align-items:center;justify-content:space-between;gap:10px;margin-bottom:10px;">
        <div>
          ...
          <div style="font-size:11px;color:#8aa0bf;margin-top:3px;">${zone.source === 'nws-alert-polygon' ? 'Official NWS alert polygon' : 'BayShield geospatial threat model'}</div>
        </div>
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px 12px;font-size:11px;color:#b7c5d9;">
        ...
        <div>Event <strong style="color:#f8fafc;font-weight:700;">${zone.event ?? 'Monitoring'}</strong></div>
        <div>Counties <strong style="color:#f8fafc;font-weight:700;">${zone.affectedCounties?.length ? zone.affectedCounties.join(', ') : 'Tampa Bay'}</strong></div>
        <div>Expires <strong style="color:#f8fafc;font-weight:700;">${zone.expires ? new Date(zone.expires).toLocaleString() : 'Active'}</strong></div>
      </div>
...
function resourceInfoContent(res: Resource, statusColor: string, pct: number) {
  return `
    <div style="min-width:220px;padding:14px;border-radius:16px;border:1px solid rgba(226,232,240,0.14);background:linear-gradient(180deg,rgba(255,255,255,0.1),rgba(255,255,255,0.03)),rgba(6,12,24,0.9);backdrop-filter:blur(16px);color:#e2e8f0;font-family:Inter,Arial,sans-serif;box-shadow:0 18px 45px rgba(2,6,23,0.35);">
      <div style="display:flex;align-items:center;justify-content:space-between;gap:10px;margin-bottom:10px;">
        <div>
...
export default function MapViewPage() {
  ...
  const zoneOverlaysRef = useRef<Array<google.maps.Marker | google.maps.Circle | google.maps.Polygon>>([]);
  const resourceOverlaysRef = useRef<Array<google.maps.Marker | google.maps.Circle>>([]);
  const liveResources: Resource[] = [
    ...shelters.filter(resource => resource.type === 'shelter'),
  ...
  useEffect(() => {
    ...
    vulnerabilityZones.forEach(zone => {
      ...
      if (zone.polygons && zone.polygons.length > 0) {
        ...
        zoneOverlaysRef.current.push(fallbackCircle);
      }
  ...
  }, [mapInstance, vulnerabilityZones]);

  useEffect(() => {
    if (!mapInstance || !window.google) return;

    resourceOverlaysRef.current.forEach(overlay => overlay.setMap(null));
    resourceOverlaysRef.current = [];

    liveResources.filter(resource => resource.type !== 'evacuation_route').forEach(resource => {
      ...
      resourceOverlaysRef.current.push(marker, circle);
    });
  }, [liveResources, mapInstance]);
  ...
  const tabs: { id: SidebarTab; label: string; icon: React.ReactNode; count?: number }[] = [
    ...
    { id: 'zones', label: 'Zones', icon: <Layers className="w-3.5 h-3.5" />, count: vulnerabilityZones.filter(zone => zone.status !== 'safe').length },
    { id: 'resources', label: 'Resources', icon: <Building2 className="w-3.5 h-3.5" />, count: shelters.filter(resource => resource.type === 'shelter').length },
  ];
  ...
  return (
    <div className="flex min-h-full flex-col overflow-hidden xl:h-full xl:flex-row">
      ...
      >
        ...
        <div className="mx-3 mt-3 rounded-xl border border-blue-400/20 bg-blue-400/10 p-3">
          <p className="text-xs font-semibold text-blue-100">Map & Evacuation = Where And How To Move</p>
          <p className="mt-1 text-[11px] leading-relaxed text-blue-100/75">
            Use this page for geography: official polygons, shelter locations, and route options. This is the spatial view, not the place for executive summary or agent debugging.
          </p>
        </div>
        ...
        <div className="overflow-hidden xl:flex-1">
                  ...
                  >
                    ...
                    {zone.affectedCounties && zone.affectedCounties.length > 0 && (
                      <div className="mt-1 text-[10px] text-slate-500">
                        {zone.event ?? 'Alert'} • {zone.affectedCounties.join(', ')}
                      </div>
                    )}
          ...
          {activeTab === 'resources' && (
            <div className="h-full max-h-[70vh] space-y-2 overflow-y-auto p-3 xl:max-h-none">
              ...
              {liveResources.filter(resource => resource.type !== 'evacuation_route').map(resource => {
                const pct = resource.capacity > 0 ? Math.round((resource.currentOccupancy / resource.capacity) * 100) : 0;
