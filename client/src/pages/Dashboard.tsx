...
import { VULNERABILITY_ZONES } from '@/lib/stormData';
...
export default function Dashboard() {
  ...
  const shelterSourceLabel = isLive
    ? shelterFeedSource === 'live_public'
      ? 'Florida Live'
      : 'Fallback'
    : 'Simulation';
  ...
  const zonesMonitoredValue = isLive
    ? hasOfficialPolygons
      ? officialPolygonZones.length
      : 0
    : VULNERABILITY_ZONES.length;
  const populationSourceLabel = isLive
    ? hasCensusBlockExposure
      ? 'Census Blocks'
      : 'No live exposure'
    : 'Simulation';
  const zoneSourceLabel = isLive
    ? hasOfficialPolygons
      ? 'Official NWS'
      : 'No polygons'
    : 'Baseline';
  ...
  const evacuateCount = isLive
    ? officialPolygonZones.filter(z => z.status === 'evacuate').length
  ...
  return (
    <div className="min-h-full space-y-4 p-4 sm:p-5">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        ...
        <div className="flex flex-col gap-2 sm:flex-shrink-0 sm:items-end">
          ...
          <div className={cn('flex items-center gap-2 px-3 py-1.5 rounded-lg border text-xs font-mono font-semibold', ts.bg, ts.border, ts.text)}>
            ...
            <span className="text-muted-foreground font-normal ml-1">· Phase {simulationPhase}/{totalPhases}</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-3 xl:grid-cols-3">
        {[
          {
            title: 'Dashboard = Command Snapshot',
            body: 'Use this page to understand the current situation fast: threat level, what changed this cycle, and whether BayShield dispatches are landing.',
            tone: 'border-cyan-400/20 bg-cyan-400/10 text-cyan-100',
          },
          {
            title: 'Best For',
            body: 'Shift handoff, executive summary, and deciding whether operators need to drill into maps, infrastructure, or agent internals.',
            tone: 'border-emerald-400/20 bg-emerald-400/10 text-emerald-100',
          },
          {
            title: 'Not For',
            body: 'Raw payload debugging, detailed resource allocation, or route-by-route evacuation planning. Those live on the other pages.',
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-4">
        {[
          ...
          },
          {
            label: 'Zones Monitored',
            value: zonesMonitoredValue,
            sub: isLive ? (hasOfficialPolygons ? 'Official alert polygons' : 'No active NWS polygons') : 'Tampa Bay region',
            ...
            <div className="flex items-center justify-between mb-3">
              <div>
                ...
                <p className="mt-0.5 text-xs text-muted-foreground">
                  Differences from the previous live cycle so operators can spot new movement fast.
                </p>
              </div>
          ...
          <div className="bg-card border border-border/50 rounded-xl p-4">
            <div className="flex items-center justify-between mb-3">
              <div>
                ...
                <p className="mt-0.5 text-xs text-muted-foreground">
                  Whether BayShield’s autonomous actions were delivered, acknowledged, or retained locally.
                </p>
              </div>
            ...
            <p className="mt-3 text-[11px] text-muted-foreground">
              {localOnlyDispatchCount > 0
                ? `${localOnlyDispatchCount} dispatch${localOnlyDispatchCount === 1 ? '' : 'es'} are still local because no outbound delivery path was confirmed.`
                : 'Outbound delivery is clear for the current live action set.'}
            </p>
          </div>
        </div>
      )}
              ...
              {(isLive ? [
                ...
                <div key={label} className="bg-background/60 rounded-lg p-3 border border-border/30">
                  <div className="flex items-center gap-1.5 mb-1.5">
                    ...
                    {isLive && <span className={cn('ml-auto text-[9px] px-1 py-0.5 rounded border font-mono', freshness.tone)}>{freshness.label}</span>}
                  </div>
          ...
          <div className="bg-card border border-border/50 rounded-xl p-4">
            ...
            <div className="relative rounded-lg overflow-hidden">
              ...
              <div className="absolute bottom-2 left-2 flex flex-wrap gap-1">
                {(isLive ? officialPolygonZones : VULNERABILITY_ZONES.filter(z => z.status !== 'safe')).slice(0, 6).map(z => (
