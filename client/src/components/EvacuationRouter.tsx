...
import { RESOURCES, VULNERABILITY_ZONES, type Resource } from '@/lib/stormData';
  ...
  return zones
    ...
    .map(z => {
      ...
      if (z.polygons && z.polygons.length > 0) {
        ...
        const maxDistance = points.reduce((largest, [lat, lng]) => {
          const distance = haversineDistance(lat, lng, z.lat, z.lng);
          return Math.max(largest, distance);
        }, 0);
        radius = Math.max(radius, Math.min(maxDistance, 12000));
      }
...
function computeFloodPenalty(steps: google.maps.DirectionsStep[], floodZoneBounds: ReturnType<typeof buildFloodZoneBounds>): { penalty: number; zones: string[] } {
  let penalty = 0;
  const zones: string[] = [];
  for (const step of steps) {
    const lat = step.start_location.lat();
    const lng = step.start_location.lng();
    for (const zone of floodZoneBounds) {
...
export default function EvacuationRouter({ map, shelters: providedShelters, zones: providedZones, onRouteSelected }: EvacuationRouterProps) {
  const shelters = (providedShelters ?? RESOURCES.filter(r => r.type === 'shelter')).filter(r => r.type === 'shelter');
  const floodZoneBounds = buildFloodZoneBounds(providedZones ?? VULNERABILITY_ZONES);
  ...
  const computeRoutes = useCallback(async (origin: google.maps.LatLng) => {
    ...
    for (const shelter of shelters) {
      ...
      try {
        ...
        const { penalty, zones } = computeFloodPenalty(leg.steps, floodZoneBounds);
